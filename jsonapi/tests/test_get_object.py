...
# def _check_articles(json):
#     assert_relationship(json['data'], 'articles')
...
async def test_logged_in_1(cli, user_1_id):
    ...
    assert_relationship(json['data'], 'articles')
    assert len(json['data']['relationships']['articles']) > 0
    for article in get_relationship(json['data'], 'articles'):
        assert_object(article, 'article')
    assert 'included' in json
    assert len(json['included']) > 0
...
@pytest.mark.asyncio
async def test_logged_in_2(cli, user_1_id, user_1_article_id_readable):
    json = await get(cli, '/articles/{}'.format(user_1_article_id_readable),
                     200, user_1_id)
